...
// Panorama is the client.Objects.AntiSpywareRule namespace.
...
// first one.
func (c *Panorama) MoveGroup(dg, prof string, movement int, rule string, e ...Entry) error {
	lister := func() ([]string, error) {
		return c.GetList(dg, prof)
	}
	...
	return c.ns.MoveGroup(c.pather(dg, prof), lister, movement, rule, names)
}
